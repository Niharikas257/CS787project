...
def findQuantity(category, cost):
    cost_per_hour = sharedData["entities"]["infrastructure"][category]["cost_per_hour"]
...
def optimizeByPercent():
    percent = optPercentInput["infrastructure"]
...
def optimizeByCost():
    cost_limit = optCostInput["infrastructure"]
    categories = ["full_time", "part_time", "contract"]
    category_costs = {}
    total_cost = 0
...
with open("answers/outOptinfrastructure.json", "w") as f:
    json.dump(output, f)
